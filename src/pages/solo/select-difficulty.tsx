  ...
  return (
    <div className="grid grid-cols-3 justify-between items-center py-1 px-2 text-sm text-white rounded-full">
      {Object.keys(DIFFICULTIES).map(diff => (
        <div key={diff}
             className={`rounded-full cursor-pointer py-2 px-6 ${DIFFICULTIES[diff as keyof typeof DIFFICULTIES] === soloDifficulty ? `text-white ${gradient} rounded-full text-md font-medium` : 'text-nord3'}`}
